...
export	const updateUser = async (SQLClient, {email, firstname, lastname, password, 
    birthdate, phone, cityLabel, postalCode, streetLabel, streetNumber, pointsNumber}, emailUpdate) => { 
    
    let query = 'UPDATE "user" SET ';
    const querySet = [];
    const queryValues = [];
    if(emailUpdate && email !== emailUpdate){
    ...
    if(queryValues.length > 0){
        queryValues.push(emailUpdate || email);
